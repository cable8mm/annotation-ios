...
class ViewController: UIViewController, UIPencilInteractionDelegate {
  ...
  @IBAction func save(_ sender: Any) {
    ...
    AF.request(
      K.API_SERVER_PREFIX + "os_findings/http_add", method: .post,
      parameters: [
      ...
      case .success(_):
        let alert = UIAlertController(
          title: "Success", message: "This action can repeat.",
          preferredStyle: UIAlertController.Style.alert)
        alert.addAction(
          UIAlertAction(
            title: "Close", style: UIAlertAction.Style.default,
      ...
      case .failure(_):
        let alert = UIAlertController(
          title: "Failed", message: "This action can repeat.",
          preferredStyle: UIAlertController.Style.alert)
        alert.addAction(
          UIAlertAction(
            title: "Close", style: UIAlertAction.Style.default,
  ...
  func viewPicture(data: JSON) {
    ...
    self.title =
      data["OsPictureType"]["name"].stringValue + " - "
      + data["OsRawPicture"]["display_name"].stringValue
    self.pictureImageView.sd_setImage(
      with: URL(string: K.API_SERVER_PREFIX + data["OsRawPicture"]["original_name"].stringValue))
    self.setStatus(status: .imageLoaded)
    guard let cgImage = self.pictureImageView.image?.cgImage?.copy() else {
      return
    }
    self.pictureImage = UIImage(
      cgImage: cgImage,
  ...
  func saveLabels(_ labels: [Int], memo: String) {
  ...
  @IBAction private func clearView(sender: Any) {
    let alert = UIAlertController(
      title: "Are you sure remove annotation data?", message: "This action cannot cancle.",
      preferredStyle: UIAlertController.Style.alert)

    // add the actions (buttons)
    alert.addAction(
      UIAlertAction(
        title: "Confirm", style: UIAlertAction.Style.destructive,
        handler: { (UIAlertAction) -> Void in
          self.canvasView.clear()
  ...
  // MARK: Image Process
  func changeInvert(_ value: Bool) {
  ...
  override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
    ...
    if segue.identifier == "showDiseaseTableViewController" {
      ...
      let targetController =
        destinationNavigationController.topViewController as! DiseaseTableViewController
    ...
    if segue.identifier == "showLabelTableViewController" {
      let destinationNavigationController = segue.destination as! UINavigationController
      let targetController =
        destinationNavigationController.topViewController as! LabelTableViewController
      targetController.pictureTypeId = self.currentPicture["OsPictureType"]["id"].intValue
    }
    ...
    if segue.identifier == "showAnnotationTableViewController" {
      let destinationNavigationController = segue.destination as! UINavigationController
      let targetController =
        destinationNavigationController.topViewController as! AnnotationTableViewController
