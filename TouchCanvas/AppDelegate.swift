...
//                    }
        ...
        AF.request(K.API_SERVER_PREFIX + "os_picture_labels.json?os_picture_type_id=1").responseJSON {response in
            switch response.result {
            case let .success(value):
        ...
        AF.request(K.API_SERVER_PREFIX + "os_picture_labels.json?os_picture_type_id=2").responseJSON {response in
            switch response.result {
            case let .success(value):
                // [{"OsPictureLabel":{"id":83,"os_picture_type_id":"2","name":"ROP||Total retinal detachment","is_polygon":true,"created":"2019-05-16 03:16:37"}},
                let osPictureLabels = JSON(value)["data"]
        ...
        AF.request(K.API_SERVER_PREFIX + "os_app_users/add.json", method: .post, parameters:parameters).responseJSON {response in
        }
