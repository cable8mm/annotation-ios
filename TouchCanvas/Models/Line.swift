...
class Line: NSObject {
  ...
  func addPointOfType(_ pointType: LinePoint.PointType, for touch: UITouch, in view: UIView)
    -> CGRect
  {
    let previousPoint = points.last
    let previousSequenceNumber = previousPoint?.sequenceNumber ?? -1
    let point = LinePoint(
      touch: touch, sequenceNumber: previousSequenceNumber + 1, pointType: pointType,
      locatedIn: view)
  ...
  func drawInContext(_ context: CGContext, isDebuggingEnabled: Bool, usePreciseLocation: Bool) {
    ...
    for point in points {
      ...
      if point.pointType.contains(.selected) {
        context.setLineWidth(K.LINE_WIDTH_SELECTED)
      } else {
      ...
      if isDebuggingEnabled && !pointType.contains(.coalesced) && !pointType.contains(.predicted)
        && !pointType.contains(.finger)
      {
        context.beginPath()
  ...
  func drawFixedPointsInContext(
    _ context: CGContext, isDebuggingEnabled: Bool, usePreciseLocation: Bool,
    ...
    if commitAll {
      ...
      for (index, point) in allPoints.enumerated() {
        ...
        guard
          point.pointType.intersection([.needsUpdate, .predicted]).isEmpty
            && index < allPoints.count - 2
        else {
    ...
    committedLine.drawInContext(
      context, isDebuggingEnabled: isDebuggingEnabled, usePreciseLocation: usePreciseLocation)
  ...
  private func updateRectForLinePoint(
    _ point: LinePoint, previousPoint optionalPreviousPoint: LinePoint? = nil
  ) -> CGRect {
    var rect = CGRect(origin: point.location, size: CGSize.zero)
...
class LinePoint: NSObject {
  ...
  struct PointType: OptionSet {
    ...
    static let selected = PointType(rawValue: 1 << 6)
  }
...
// Line에 selected 속성을 전체 적용하거나 없앤다. Line을 선택했을 때 굵게 표기하기 위함.
extension Line {
  //    private var points = [LinePoint]()
  func addSelectedProperty() {
    for (_, point) in committedPoints.enumerated() {
      point.pointType.formUnion(.selected)
    }
  }

  func removeSelectedProperty() {
    for (_, point) in committedPoints.enumerated() {
      point.pointType.subtract(.selected)
    }
  }

  func getDrawPoints() -> [DrawPoint] {
    let drawPoints = committedPoints.map({ (value: LinePoint) -> DrawPoint in
      return DrawPoint(
        sequenceNumber: value.sequenceNumber, timestamp: value.timestamp, x: value.location.x,
        y: value.location.y)
    })
    return drawPoints
  }
