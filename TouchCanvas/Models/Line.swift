...
//    private var points = [LinePoint]()
    ...
    func getDrawPoints() -> [DrawPoint] {
        let drawPoints = committedPoints.map({ (value: LinePoint) -> DrawPoint in return DrawPoint(sequenceNumber: value.sequenceNumber, timestamp: value.timestamp, x: value.location.x, y: value.location.y) })
        return drawPoints
    }
}
